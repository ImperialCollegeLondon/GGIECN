"""
Created on Mon Sep 11 12:55:29 2023
...
class RegularStream_sub1(nn.Module):
...
class RegularStream_sub2(nn.Module):
    def __init__(self, backbone_type='resnet50'):
        super().__init__()
        if backbone_type == 'resnet50':
            self.backbone = resnet50d(in_chans=2,output_stride=8)
        else:
            self.backbone = resnet101d(in_chans=2,output_stride=8)
...
class GGIECN_sub1(nn.Module):
    def __init__(self, backbone_type='resnet50', num_classes=3):
        super().__init__()

        self.regular_stream = RegularStream_sub1(backbone_type)
...
class GGIECN_sub2(nn.Module):
    def __init__(self, backbone_type='resnet50', num_classes=3):
        super().__init__()

        self.regular_stream = RegularStream_sub2(backbone_type)
        self.shape_stream = ShapeStream()
        self.feature_fusion = FeatureFusion(2048, (12, 24, 36), 256)
        self.seg = nn.Sequential(
            nn.Conv2d(256 + 48, 256, kernel_size=3, padding=1, bias=False),
